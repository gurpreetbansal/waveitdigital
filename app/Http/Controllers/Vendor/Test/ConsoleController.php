...
class ConsoleController extends Controller {
	...
	public function check_searchConsole_cron(){
		 try{
            ...
            ->whereNotNull('console_account_id')
            ->orderBy('id','desc')
            ->where('status',0)
            ->get();       
            ...
            if(!empty($getUser)){
                file_put_contents(dirname(__FILE__).'/logs/GoogleSearchConsole_data_1.txt',print_r($getUser,true));
                ...
                foreach($getUser as $key=>$data){
                    ...
                    if(!empty($getAnalytics)){
                        ...
                        $check = SearchConsoleUrl::checkConsoleData_cron($client,$data->id,$data->user_id,$data->google_console_id,$data->console_account_id);
        ...
        private function search_console_graph_data($client,$profileUrl,$start_date,$end_date,$campaignId){
        	...
        	GoogleUpdate::updateTiming($campaignId,'search_console','sc_type',1);
        }
